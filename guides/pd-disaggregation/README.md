...
The `bash` commands below downloads the benchmark runner script (`run_only.sh`), then presents an interactive menu of PD (Prefix-Disaggregation) benchmark templates from the llm-d repository's [`guides/benchmark/`](guides/benchmark/) directory. Once the user selects a template, it downloads that specific YAML configuration file for running benchmarks.
